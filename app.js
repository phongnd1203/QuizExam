...
const categoryBreakdownContainer = document.getElementById(
  "category-breakdown-container",
);
const categoryBreakdownList = document.getElementById(
...
const shuffleQuestionsToggle = document.getElementById(
...
let shouldShuffleQuestions =
  localStorage.getItem("shuffleQuestions") !== "false";
...
// Load Quizzes List from metadata (no 404 scanning loop)
async function loadQuizzes() {
  if (!quizListContainer) return;
  quizListContainer.innerHTML =
    '<p style="color: var(--text-muted); font-size: 0.9rem;">Đang tải danh sách bài thi...</p>';
  let quizzes = [];

  if (window.PMG_QUIZZES && Array.isArray(window.PMG_QUIZZES)) {
    quizzes = window.PMG_QUIZZES;
  } else {
    try {
      ...
      console.warn("Could not fetch data/quizzes.json", error);
    }
  }
...
function handleModeSelect(mode) {
  ...
  if (
    ...
    savedProgress.userAnswers.some((a) => a !== null)
  ) {
    if (resumeModalDesc) {
...
function prepareShuffledQuiz(
  ...
  if (shuffleOptions) {
    data.questions.forEach((q) => {
      ...
      const optionsWithIndex = originalOptions.map((opt, i) => ({
        text: opt,
        originalIndex: i,
      }));
      ...
      if (isMultiple) {
        q.answer = q.answer.map((origAns) =>
          optionsWithIndex.findIndex((o) => o.originalIndex === origAns),
        );
      } else {
        q.answer = optionsWithIndex.findIndex(
...
async function startQuiz(quizMeta, mode, resume) {
  try {
    ...
    if (
      resume &&
      savedProgress &&
      savedProgress.shuffledQuizData &&
      savedProgress.shuffledQuizData.questions
    ) {
...
function renderAllQuestions() {
  if (!questionsContainer || !navGrid || !quizData || !quizData.questions)
    return;
  ...
  quizData.questions.forEach((qData, qIndex) => {
    ...
    currentMultipleSelections[qIndex] = Array.isArray(userAnswers[qIndex])
      ? [...userAnswers[qIndex]]
      : [];
    ...
    if (currentQuizMode === "practice") {
      qTag.textContent = isMultiple
        ? `Câu ${qIndex + 1} • Chọn ${requiredCount} đáp án`
        : `Câu ${qIndex + 1} • Chọn 1 đáp án`;
    } else if (currentQuizMode === "review") {
      qTag.textContent = isMultiple
        ? `Câu ${qIndex + 1} • ${requiredCount} đáp án đúng`
        : `Câu ${qIndex + 1} • 1 đáp án đúng`;
    } else {
      qTag.textContent = isMultiple
        ? `Câu ${qIndex + 1} • Chọn nhiều đáp án`
        : `Câu ${qIndex + 1} • Chọn 1 đáp án`;
    }
    ...
    flagBtn.innerHTML = `<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"></path></svg>`;
    ...
    if (currentQuizMode === "practice" && isMultiple) {
      ...
      if (userAnswers[qIndex] !== null) {
        const isCorrect =
          ...
          qData.answer.length === userAnswers[qIndex].length &&
          qData.answer.every((a) => userAnswers[qIndex].includes(a));
        hintEl.textContent = isCorrect
          ? " Trả lời chính xác"
          : " Chưa chính xác";
    ...
    qData.options.forEach((opt, optIndex) => {
      ...
      if (currentQuizMode === "review") {
        ...
        const isSelected = isMultiple
          ? Array.isArray(userAnswers[qIndex]) &&
            userAnswers[qIndex].includes(optIndex)
          : userAnswers[qIndex] === optIndex;
        const isCorrectChoice = isMultiple
          ? qData.answer.includes(optIndex)
          : qData.answer === optIndex;

        if (isCorrectChoice) {
          btn.classList.add("correct");
          btn.insertAdjacentHTML(
            "beforeend",
            `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="color:var(--success);flex-shrink:0"><polyline points="20 6 9 17 4 12"></polyline></svg>`,
          );
        } else if (isSelected && !isCorrectChoice) {
          btn.classList.add("incorrect");
          btn.insertAdjacentHTML(
            "beforeend",
            `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="color:var(--danger);flex-shrink:0"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>`,
          );
        }
        ...
        if (userAnswers[qIndex] !== null) {
          ...
          btn.addEventListener("click", () => {
            if (isMultiple) {
              const selIdx =
                currentMultipleSelections[qIndex].indexOf(optIndex);
              if (selIdx > -1) {
    ...
    if (qData.explanation) {
      ...
      const showExp =
        ...
        (currentQuizMode === "practice" && isAnswered);
...
function handlePracticeSingleSelect(qIndex, optIndex, optsContainer) {
  ...
  if (optIndex === qData.answer) {
    ...
    if (allOptions[qData.answer]) {
      ...
      allOptions[qData.answer].insertAdjacentHTML(
        "beforeend",
        `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="color:var(--success);flex-shrink:0"><polyline points="20 6 9 17 4 12"></polyline></svg>`,
      );
    }
...
function handlePracticeMultipleSelect(qIndex, optsContainer, qFooter) {
  ...
  const isAllCorrect =
    correctAnswers.length === selection.length &&
...
function handleExamOptionSelect(qIndex, optIndex, isMultiple, optsContainer) {
  ...
  if (isMultiple) {
    ...
    allOptions.forEach((btn, idx) => {
      if (
        Array.isArray(userAnswers[qIndex]) &&
        userAnswers[qIndex].includes(idx)
      ) {
        btn.classList.add("selected");
      } else {
        btn.classList.remove("selected");
...
function updateNavStyles() {
  if (!quizData || !quizData.questions) return;
  let answeredCount = 0;
  const isPracticeOrReview =
    currentQuizMode === "practice" || currentQuizMode === "review";
  const filtered = getFilteredQuestionIndices();
    ...
    if (uAns !== null && uAns !== undefined) {
      ...
      if (isPracticeOrReview) {
        ...
        if (Array.isArray(qData.answer)) {
          isCorrect =
            Array.isArray(uAns) &&
            qData.answer.length === uAns.length &&
            qData.answer.every((a) => uAns.includes(a));
        } else {
  ...
  if (progressText) {
    if (currentCategory === "all") {
      ...
      const filteredAnswered = filtered.filter(
        (i) => userAnswers[i] !== null && userAnswers[i] !== undefined,
      ).length;
  ...
  submitQuizBtn.addEventListener("click", () => {
    ...
    if (unanswered > 0) {
      const confirm = window.confirm(
        `Bạn còn ${unanswered} câu chưa hoàn thành. Bạn có chắc chắn muốn nộp bài không?`,
      );
      if (!confirm) return;
    }
    finishQuiz();
...
function finishQuiz() {
  ...
  quizData.questions.forEach((q, idx) => {
    ...
    if (Array.isArray(q.answer)) {
      if (
        ...
        q.answer.every((a) => uAns.includes(a))
      ) {
        score++;
      }
    } else {
  ...
  const bestScore =
    savedProgress.bestScore !== undefined
      ? Math.max(savedProgress.bestScore, score)
      : score;
  ...
  if (resultBadgeText) {
    resultBadgeText.textContent =
      currentQuizMode === "practice" ? "Kết quả học tập" : "Kết quả kiểm tra";
  }
...
if (retryBtn) {
  retryBtn.addEventListener("click", () => {
    startQuiz(
      pendingQuizMeta,
      currentQuizMode === "review" ? "exam" : currentQuizMode,
      false,
    );
  });
}
...
function initTabNavigation() {
  const tabBtns = {
    articles: document.getElementById("nav-tab-articles"),
    quizzes: document.getElementById("nav-tab-quizzes"),
    glossary: document.getElementById("nav-tab-glossary"),
    eos: document.getElementById("nav-tab-eos"),
  };

  const tabPanes = {
    articles: document.getElementById("articles-tab-content"),
    quizzes: document.getElementById("quizzes-tab-content"),
    glossary: document.getElementById("glossary-tab-content"),
  };

  function switchTab(tabName) {
    Object.keys(tabBtns).forEach((key) => {
      if (tabBtns[key]) {
        tabBtns[key].classList.toggle("active", key === tabName);
      }
    });

    Object.keys(tabPanes).forEach((key) => {
      if (tabPanes[key]) {
        tabPanes[key].classList.toggle("hidden", key !== tabName);
      }
    });
  ...
  if (tabBtns.articles) {
    tabBtns.articles.addEventListener("click", () => switchTab("articles"));
  }
  if (tabBtns.quizzes) {
    tabBtns.quizzes.addEventListener("click", () => switchTab("quizzes"));
  }
  if (tabBtns.glossary) {
    tabBtns.glossary.addEventListener("click", () => switchTab("glossary"));
  }
  if (tabBtns.eos) {
    tabBtns.eos.addEventListener("click", () => {
      switchTab("articles");
      if (typeof openArticle === "function") {
        openArticle("guide-eos-tips");
      }
    });
  }
  ...
  if (appLogo) {
    appLogo.addEventListener("click", () => switchTab("articles"));
  }
}
